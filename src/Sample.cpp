...
void nrook(Vector2* samples, int num_samples)
{
	for (int i = 0; i < num_samples; i++)
	{
...
void multiJitter(Vector2* samples, int num_samples)
{
	...
	// shuffle x coords within each column and y coords withineach row
	for (int i = 0; i < sqrt_samples; i++)
	{
		for (int j = 0; j < sqrt_samples; ++j)
		{
			...
			t = samples[i * sqrt_samples + j].e[1];
			samples[i * sqrt_samples + j].e[1] = samples[i * sqrt_samples + k].e[1];
			samples[i * sqrt_samples + k].e[1] = t;			
		}
	}
}
